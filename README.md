...
```bash
# 克隆仓库（含题目数据 submodule）
git clone --recursive https://github.com/yhy0/xbow-benchmark-agent.git
cd xbow-benchmark-agent

# 如果已经 clone 过但缺少 submodule
git submodule update --init --depth 1
```

> 题目数据位于 `xbow-validation-benchmarks/benchmarks/`，来自 [Neuro-Sploit/xbow-validation-benchmarks](https://github.com/Neuro-Sploit/xbow-validation-benchmarks)。
...
# 启动服务（题目数据已内置，无需指定 --xbow-benchmark-folder）
# --host: API 监听地址
# --port: API 监听端口
# --public-accessible-host: 赛题对外暴露的 IP (通常为本机局域网 IP，如：192.168.1.2)
# -i: 指定启动的题目 ID，可重复指定。省略则空启动，后续通过 /admin/start API 动态加题
...
* **Dashboard**: http://127.0.0.1:8000/dashboard — 多 Agent Benchmark 对比看板
* **Swagger UI**: http://127.0.0.1:8000/docs
* **运行日志**: logs/competition-platform-server-logs.jsonl

![fastapi](./figures/fastapi.png)

## Dashboard（多 Agent 对比看板）

启动服务器后访问 `/dashboard`，可实时查看多个 Agent 跑 Benchmark 的进度与对比：

* **Summary 卡片**：Solved / Score / Cost / Avg Time / Tokens / Running 一目了然
* **Leaderboard**：所有 Agent 按 Solved 和总分排名
* **Runs 明细**：每道题的状态（✅OK / ❌FAIL / ⏳RUN / ⏱T/O / ⚠ERR）、用时、费用、Token 消耗
* **Agent 筛选**：点击顶部 Agent Tab 切换单 Agent 视图
* **自动刷新**：每 10 秒轮询更新

**技术栈**：纯静态 HTML + JS（无框架），Glassmorphism 暗色主题，SQLite 持久化。

### Benchmark Runner 集成

```bash
# 跑 benchmark 时指定 agent 名称（用于 Dashboard 区分）
uv run python scripts/benchmark_runner.py --benchmarks 1,2,3 --agent-name alice
uv run python scripts/benchmark_runner.py --range 1-20 --agent-name bob-v2
```

Runner 会在每道题开始/结束时自动向 Dashboard API 上报进度。
...
| POST | /api/v1/answer | 提交 Flag，校验通过后返回积分并标记完成。支持 `agent_name` 字段。 |
| POST | /api/v1/admin/start | 动态启动题目容器（`{"benchmark_ids": [1, 2]}`）。 |
| POST | /api/v1/admin/stop | 停止并清理单个容器（`{"challenge_code": "uuid"}`）。 |
| POST | /api/v1/admin/stop_all | 停止并清理所有容器。 |

### Dashboard 接口

| **方法** | **路径** | **描述** |
| --- | --- | --- |
| POST | /api/v1/runs/start | 记录一道题开始运行（status=running）。 |
| POST | /api/v1/runs/finish | 记录一道题运行结果（status/duration/cost/tokens）。 |
| GET | /api/v1/dashboard/agents | 获取所有 Agent 名称列表。 |
| GET | /api/v1/dashboard/summary | 按 Agent 汇总统计（?agent_name=xxx 可选）。 |
| GET | /api/v1/dashboard/runs | 获取 Run 记录明细（?agent_name=xxx&limit=200）。 |
| GET | /api/v1/dashboard/leaderboard | 排行榜（所有 Agent 按 Solved DESC, Score DESC）。 |
